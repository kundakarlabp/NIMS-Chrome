...
''',
        '''        // Runtime assets are loaded and registered by NimsWebViewRuntime below.
''',
...
''',
        '''        when (json.optString("type")) {
            "nims_runtime_ready" -> {
                ...
                log(
                    "Runtime frame=$path ready=$ready jquery=${jqueryVersion.ifBlank { "none" }} " +
                        "fallback=${json.optBoolean("jqueryFallbackUsed")} offset=${json.optBoolean("offsetPatched")} " +
                        "tab=${json.optBoolean("ajaxCompleteTabPatched")}"
                )
                if (ready && path.startsWith("/HISInvestigationG5/") &&
                    appStateValue == AppState.HELPER_READY && activeProcessingJob?.isActive != true) {
