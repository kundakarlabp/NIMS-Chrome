...
class SecureSettings @JvmOverloads constructor(
    context: Context,
    private val crypto: SettingsCrypto = AndroidKeyStoreSettingsCrypto,
    preferencesName: String = "nims_mobile_settings"
) {
    private val prefs = context.getSharedPreferences(preferencesName, Context.MODE_PRIVATE)
    private val clinical = EncryptedClinicalPreferences(
    ...
    fun apiKey(): String = decryptPref("helper_key", HELPER_KEY_ALIAS, clearOnFailure = { clearApiKey() })
    ...
    private fun migrateClinicalData() {
        clinical.migrate(KEY_LAST_SUMMARY_ENC, "last_summary_json")
        clinical.migrate(KEY_NOTE_ENC, "physician_note")
    }

    private fun decryptPref(key: String, alias: String, clearOnFailure: () -> Unit): String {
        val value = prefs.getString(key, "") ?: ""
        if (value.isBlank()) return ""
        return try {
            crypto.decrypt(decodeEncrypted(value), alias)
        } catch (_: Exception) {
            clearOnFailure(); ""
        }
    }

    private fun encodeEncrypted(value: String): String = "enc:v1:$value"
    private fun decodeEncrypted(value: String): String = if (value.startsWith("enc:v1:")) value.removePrefix("enc:v1:") else value
