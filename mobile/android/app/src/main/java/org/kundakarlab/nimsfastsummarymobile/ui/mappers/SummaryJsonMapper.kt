...
object SummaryJsonMapper {
            ...
            for (index in 0 until rows.length()) {
                ...
                add(
                    UiSourceReport(
                        ...
                        hasError = status.equals("error", ignoreCase = true) ||
                            status.equals("unsupported", ignoreCase = true),
    ...
    private fun sirSummary(row: JSONObject): String {
        ...
        val susceptible = valuesFromKeys(
            row,
            "sensitive",
            "susceptible",
            "sensitive_drugs",
            "susceptible_antibiotics"
        ) + valuesFromKeys(summaryObject, "sensitive", "susceptible")
        val intermediate = valuesFromKeys(
            row,
            "intermediate",
            "intermediate_drugs",
            "intermediate_antibiotics"
        ) + valuesFromKeys(summaryObject, "intermediate")
        val resistant = valuesFromKeys(
            row,
            "resistant",
            "resistant_drugs",
            "resistant_antibiotics"
        ) + valuesFromKeys(summaryObject, "resistant")
        ...
        return listOf(
            ...
            values.distinctBy { it.lowercase() }
    ...
    private fun valuesFromKeys(row: JSONObject?, vararg keys: String): List<String> {
        ...
        return buildList {
            for (key in keys) addAll(stringValues(row.opt(key)))
        }
    }
    ...
    private fun cultureComment(row: JSONObject): String {
        val notes = buildList {
            firstNonBlank(row, "comment", "microbiology_note", "note")
                .takeIf { it.isNotBlank() }
                ?.let(::add)
            firstNonBlank(row, "bottle_name")
                .takeIf { it.isNotBlank() }
                ?.let { add("Bottle: $it") }
            firstNonBlank(row, "reporting_status", "report_status")
                .takeIf { it.isNotBlank() }
                ?.let { add("Report status: $it") }
            firstNonBlank(row, "isolate_number")
                .takeIf { it.isNotBlank() }
                ?.let { add("Isolate: $it") }
            if (booleanField(row, "clinical_review_flag")) {
                add("Clinical-significance review required; verify with source report and bedside context.")
            }
        }
        return notes.distinct().joinToString(" | ")
    }

    private fun booleanField(row: JSONObject, key: String): Boolean {
    ...
    private fun stringsPreserveBlanks(values: JSONArray?): List<String> {
        ...
        return buildList {
            for (index in 0 until values.length()) {
                add(if (values.isNull(index)) "" else values.optString(index))
            }
        }
    }
