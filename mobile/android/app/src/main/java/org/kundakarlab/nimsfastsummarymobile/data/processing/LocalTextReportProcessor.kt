...
object LabTextParser {
    private val defs = listOf(
        ...
        LabDefinition("CRP", "CRP", labels("CRP"), setOf("mg/L"), 0.0..1000.0),
    ...
    private fun parseLine(line: String, date: String?): ParsedLabValue? {
        ...
        val result = Regex("[:=]?\\s*([<>])?\\s*([0-9][0-9,]*(?:\\.[0-9]+)?)\\s*([a-zA-Z/%µ]+(?:/[a-zA-Zµ]+)?|lakh/cumm|cells/cumm|mmol/L|mEq/L|ng/mL|mg/L|mg/dL|g/dL|gm%|U/L)?", RegexOption.IGNORE_CASE).find(remaining) ?: return null
...
object CultureTextParser {
    ...
    private fun parseBlock(block: String, date: String?): ParsedCultureValue? {
        ...
        val susceptibility = Regex("([A-Za-z][A-Za-z /-]{2,30})\\s+((?:Sensitive|Susceptible|Intermediate|Resistant))", RegexOption.IGNORE_CASE).findAll(block).map { AntibioticResult(it.groupValues[1].trim(), it.groupValues[2].trim(), ParseConfidence.MEDIUM) }.toList()
