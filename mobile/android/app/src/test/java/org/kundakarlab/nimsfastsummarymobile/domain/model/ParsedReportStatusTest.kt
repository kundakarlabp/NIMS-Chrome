...
class ParsedReportStatusTest {
    ...
    fun fullyParsedReportHasExplicitStatusAndProvenance() {
        ...
        val processing = report.toHelperJson().getJSONObject("processing")
    ...
    fun structuredReportWithWarningsIsPartial() {
        val report = report(labs = listOf(minimalLab()), warnings = listOf("Platelet row was not extracted"))
    ...
    fun rawTextWithoutStructuredValuesIsUnstructured() {
        assertEquals(ReportParseStatus.UNSTRUCTURED, report(rawText = "Complete blood picture report text").parseStatus)
    }

    @Test
    fun sessionAndFetchFailuresAreNotSilentlyUnstructured() {
        assertEquals(ReportParseStatus.SESSION_EXPIRED, report(warnings = listOf("NIMS session appears expired. Login again.")).parseStatus)
        assertEquals(ReportParseStatus.FETCH_FAILED, report(warnings = listOf("Report fetch failed with HTTP 500")).parseStatus)
        assertEquals(ReportParseStatus.UNSUPPORTED, report(warnings = listOf("Unsupported report format")).parseStatus)
    }

    @Test
    fun helperJsonRetainsWarningsForClinicalVerification() {
        val warning = "Only Hb was extracted; verify original PDF"
        val processing = report(labs = listOf(minimalLab()), warnings = listOf(warning)).toHelperJson().getJSONObject("processing")
