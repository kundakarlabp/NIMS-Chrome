...
test('preserves valid offset coordinates', () => {
  const jq = () => {};
  jq.fn = { offset: () => ({ left: 12, top: 8 }) };
  const win = fixture({ jQuery: jq, $: jq });
  win.flush();
  assert.equal(win.jQuery.fn.offset().left, 12);
  assert.equal(win.jQuery.fn.offset().top, 8);
});
